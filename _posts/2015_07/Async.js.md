title: async.js(转载)
date: 2015-07-22 14:26:35
updated: 2015-07-22 14:26:37
tags:
- node.js
- js
...
async.eachSeries(hugeArray, function iterator(item, callback) {
  if (inCache(item)) {
    async.setImmediate(function () {//防止爆栈
...
<a name="series" />
tasks中所有函数串行执行，函数签名如下：
function(cb){
    // do some stuff ...
    cb(null, 'one');
}
通过调用cb告诉async.js执行下一个回调函数，若cb返回错误，则停止所有tasks的执行，并执行最后一个回调函数
总结：一个函数中多个代码段可以拆分成多个函数，放在tasks中，由async.js执行
一般tasks中每个函数都执行IO操作，在IO完成时调用cb continue
...
相当于while，第一个函数为条件，第二个函数为循环体，while循环结束时，执行第三个函数
...
与whilst类似，唯一的区别是test函数可以是个async函数
...
fn是个async函数，fn会永远执行，直到fn调用callback(err,...)
